...
export default function TournamentRegisterForm({
    ...
    return (
      <div className="shine-border rounded-[1.35rem]">
        <div className="shine-border-inner rounded-[1.35rem] bg-[#0a1020]/80 p-6 backdrop-blur-sm">
          ...
          <p className="mt-3 text-sm leading-relaxed text-white/55">Sign in to lock your spot.</p>
          <Link href={`/login?callbackUrl=/esports/tournaments/${slug}`} className="cta mt-5 inline-flex w-full items-center justify-center rounded-full py-3 text-xs font-semibold uppercase tracking-[0.18em]">
            Sign in to register
          </Link>
        </div>
      </div>
    ...
    try {
      const res = await fetch(`/api/tournaments/${slug}/register`, {
        ...
        body: JSON.stringify({
          ...
          partnerAccountId: partnerAccountId.trim().toUpperCase(),
        }),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  async function submitRegistration() {
    if (submitting.current || loading || !participantRole) return;
    submitting.current = true;
    setLoading(true);
    setError(null);
    ...
    try {
      const body =
        participantRole === "CAPTAIN"
          ? { participantRole, teamName: teamName.trim(), logoUrl: logoUrl! }
          : { participantRole };
  ...
  function ProfilePreview() {
    ...
    return (
      <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-sm text-white/70">
        {preview.displayName ? <p className="font-medium text-white/90">{preview.displayName}</p> : null}
        {game === "EA_FC26" && preview.accountId ? (
          <p className="font-mono text-[var(--color-brand)]/80">{preview.accountId}</p>
        ) : null}
        ...
        <div className="shine-border-inner space-y-4 rounded-[1.35rem] bg-[#0a1020]/85 p-6 backdrop-blur-sm">
          <div>
            ...
            <p className="mt-2 text-sm text-white/45">
              Register your 2v2 team. Your partner must have an NTG account — use their Account ID from their profile.
            </p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <input
              className={inputClass}
              placeholder="Partner Account ID (e.g. NTG1234)"
              value={partnerAccountId}
              onChange={(e) => setPartnerAccountId(e.target.value.toUpperCase())}
            />
            <p className="text-xs text-white/40">
              Share your Account ID ({preview?.accountId ?? "see profile"}) with your partner so they can find you too.
            </p>
            <button
              type="button"
              onClick={submitFifaRegistration}
              disabled={loading || !teamName.trim() || !partnerAccountId.trim()}
        ...
        {step === "role" && (
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            >
              ...
              <p className="mt-1 text-xs text-white/40">Create a team with name & logo</p>
            </button>
            <button
              type="button"
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <button type="button" onClick={() => fileRef.current?.click()} disabled={logoUploading} className="w-full rounded-xl border border-dashed border-white/15 py-3 text-xs uppercase tracking-wider text-white/55 hover:border-white/30">
                {logoUploading ? "Uploading…" : logoUrl ? "Logo uploaded — change" : "Upload team logo (max 10 MB)"}
              </button>
        ...
        {step === "confirm" && (
          <div className="space-y-3">
            <p className="text-sm text-white/60">
              ...
              {participantRole === "CAPTAIN" && teamName ? <> — <strong className="text-white">{teamName}</strong></> : null}
            </p>
            <div className="flex gap-2">
              ...
              <button type="button" onClick={submitRegistration} disabled={loading} className="cta flex-1 rounded-full py-3 text-xs font-semibold uppercase tracking-[0.18em] disabled:opacity-50">
