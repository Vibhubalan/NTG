...
export default function TournamentGamesSection({ slug, initialGames }: Props) {
  ...
  return (
    <div className="space-y-6 font-sans">
      {/* Match Cards List Tiles (Neutral Design) */}
                ...
                <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#080d16] shadow-2xl backdrop-blur-xl transition-all duration-500 animate-in fade-in slide-in-from-top-4">
                  ...
                  <div className="flex items-center justify-between border-b border-white/10 bg-black/40 px-6 py-4">
                    ...
                    <button
                      ...
                      className="rounded-full border border-white/15 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white/60 hover:text-white hover:bg-white/10"
                    >
                      Close Scoreboard
                    </button>
                  </div>
                  ...
                  <div className="p-4 sm:p-6 space-y-6">
                    {[
                      {
                        ...
                        headerTint: "bg-white/[0.04] text-white",
                      },
                      {
                        teamKey: "teamB",
                    ...
                    ].map(({ teamKey, team, borderColor, headerTint }) => (
                      <div key={teamKey} className={`overflow-hidden rounded-xl border ${borderColor} bg-[#060b13] shadow-lg`}>
                        {/* Team Section Header */}
                        <div className={`flex items-center justify-between px-4 py-3 ${headerTint} border-b border-white/10`}>
                          ...
                          <table className="w-full min-w-[640px] text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-white/[0.04]">
                              {team.players.map((p) => {
                                ...
                                const agentIcon = getAgentIconUrl(p.agent);
                                const kdRatio = p.deaths === 0 ? p.kills : p.kills / p.deaths;
                                const isMvp =
                                  !!selected.mvpRiotId &&
                                  p.riotId.toLowerCase() === selected.mvpRiotId.toLowerCase();
                                ...
                                return (
                                  ...
                                  >
                                      ...
                                      <div className="flex items-center gap-3">
                                        ...
                                        <div className="min-w-0 flex-1">
                                          <div className="flex items-center gap-1.5">
                                            ...
                                            {isMvp ? (
                                              <span className="rounded bg-emerald-400/20 px-1.5 py-0.2 text-[9px] font-black uppercase text-emerald-300 ring-1 ring-emerald-400/30">
                                                MVP
                                              </span>
                                            ) : null}
                                          </div>
                                    ...
                                    <td className="py-2.5 px-2 text-center align-middle font-mono font-bold text-sm">
                                      <span className={kdRatio >= 1.2 ? "text-emerald-400" : kdRatio < 0.8 ? "text-rose-400" : "text-white/80"}>
                                        {kdRatio.toFixed(1)}
                                      </span>
                                    </td>
